...
//
// IMPROVED THRESHOLDS for faster & more accurate detection:
// - Reduced frame threshold: 3 frames × 200ms = 0.6s (was 0.8s)
// - Lowered probability gates for earlier detection
// - Better class-specific thresholds for yawning/fatigue/microsleep
const int _kDrowsyThreshold = 3; // ↓ from 4 — faster response (0.6s)
const double _kDrowsyPctGate = 15.0; // ↓ from 18.0 — more sensitive
...
const Map<int, double> _kBehaviorClassThresholds = {
  ...
  // IMPROVED: Lower thresholds for faster yawning/fatigue detection
  9: 4.0, // drowsy_yawning          ↓ from 6.0  — yawning is very distinctive
  10: 6.0, // drowsy_yawning_occluded ↓ from 8.0  — partially visible yawn
  11: 3.5, // drowsy_fatigue          ↓ from 5.0  — EAR decrease + MAR combined
  ...
  InferenceResult _buildResult(
    ...
    if (bestDistIdx == 7 && bestDistScore < 55.0) {
      ...
      if (secondBestScore >= bestDistScore - 20.0) {
        groomingIsFP = true;
        debugPrint(
            '[GroomingFP] demoted: grooming=${bestDistScore.toStringAsFixed(1)}% '
    ...
    final finalClassMinThreshold =
        _kBehaviorClassThresholds[bestDistIdx] ?? 5.0;
    ...
    final finalDrowsyMinThreshold =
        _kBehaviorClassThresholds[bestDrowsyIdx] ?? 6.0;
    ...
    final bool distractedConfirmed =
        _consecutiveDistracted >= effectiveDistThreshold;
